...
- 危険性の高い操作だけは止めてほしい
- `--dangerously-bypass-approvals-and-sandbox` で安全機構をすべて解除する設定は、常用したくない
...
安全設定が厳しすぎると、通常作業でも確認や失敗が頻発する。その結果、利用者が安全機構を段階的に調整するのではなく、面倒を避けるために制限解除オプションを選ぶ可能性が高くなる。
...
- 制限解除オプションは通常設定と視覚的・操作的に分離する

## プリセット

### 1. 承認を求める

ワークスペース内の開発とネットワーク利用を許可し、安全でない可能性がある操作はユーザーへ確認する。
...
- Auto-review に委譲された `PermissionRequest` は内部履歴だけに記録し、ユーザー待ちを表す外部 Webhook へは送信しない

表示例：

```text
代理で承認
通常作業を自動化し、安全でない可能性がある操作は
AIが代理で審査します。
```
...
`承認を求める` と `代理で承認` は同じ作業範囲を設定し、承認者だけを切り替える。
...
Elevated のセットアップに失敗した場合、TerminalHub が利用者へ知らせず Unelevated や制限解除オプションへ切り替えてはならない。

- 現在の実効モードを表示する
- セットアップ失敗理由を表示する
- Unelevated で再試行する選択肢を提示する
- 制限解除オプションへの自動切り替えは行わない
...
- 警告を確認しないと有効化できない
- 有効中であることが常時表示される
- TerminalHub が自動的に制限解除オプションへ切り替えない
...
一般利用者にはこのプリセットを提示し、ネットワークを禁止したい場合や承認方式を変更したい場合だけ詳細設定を使用する。サンドボックスと承認を完全に無効化する `--dangerously-bypass-approvals-and-sandbox` は、明確な警告付きの例外的な選択肢として扱う。
