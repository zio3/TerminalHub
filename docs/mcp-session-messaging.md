...
>
> 親が**この一覧に載っていないこともある**。理由は3つあり、(1) 親が削除された、(2) 親がアーカイブされた（`GetActiveSessions()` が `IsArchived` を除外するため一覧から消える）、(3) 呼び出し側のフィルタ引数で単に落ちた（フィルタは1件ずつ独立に効くので、子が残っても親が落ちることがある。親は生きている）。
>
> いずれの場合も、その GUID を宛先に使って**別セッションへ誤送信されることはない**。削除済みなら `send_to_session` が「宛先が見つかりません」で弾き、アーカイブ済みなら `GetSessionInfo()` では引けるものの ConPTY が無いので「未起動」で弾かれる。
