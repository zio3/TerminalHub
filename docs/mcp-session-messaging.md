...
| エンベロープあり | `proof` を渡すと、**検証済みの送信元**（表示名＋GUID）を本文末尾にサーバーが付与する。自己申告の GUID は受け付けない |
...
| 条件 | 付与される文字列 |
|---|---|
| `proof` + `contextId` | `[TerminalHub — 依頼元: <名前> (<GUID>) / contextId: <id> — 完了時は update_context に status と結果を書く。途中で相談したいときだけ send_to_session で依頼元へ返す]` |
| `proof` のみ | `[TerminalHub — 依頼元: <名前> (<GUID>) — 返信は send_to_session でこの GUID へ]` |
| `contextId` のみ | `[contextId: <id> — 状況・結果は update_context で共有]` |
