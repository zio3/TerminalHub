...
TerminalHub 自身が発するシステム通知（完了通知・配送失敗通知）にも同じ形式のエンベロープが付く（`送信元: TerminalHub (system)`・配送記録にも残る）。「マーカー無し＝人間」の規則を TerminalHub 自身の通知が破らないため。

**唯一の例外: `/` で始まる本文（スラッシュコマンド送信）にはエンベロープを付けない。**（そのため **contextId とは併用不可**＝送信時に拒否される。配送記録は残り、`deliveryId` は送信結果で送信者に返る＝受け手はエンベロープが無いため引けないが、送信側からの監査は成立する） 末尾の角括弧がコマンドの引数として解釈され、引数を取るコマンド（`/model` 等）を壊すため（`/context` のような引数無視のコマンドは付いても動くことを実機確認済みだが、一貫して付けない）。判定は行頭の `/` のみで Trim しない（先頭に空白があれば TUI 側もコマンドとして扱わない）。「マーカー無し＝人間」の保証にはこの分だけ穴が開くが、`/` で始まる入力は CLI にコマンドとして解釈され通常プロンプトにならないため、指示のなりすましには実質使えない。ローカル利用前提で許容（2026-08-01 判断）。**配送記録（`Deliveries`）は例外なく残す**＝誰がどのセッションへコマンドを送ったかの監査は保たれる。
