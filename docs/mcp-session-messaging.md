...
| `parentSessionId` | string? | worktree セッションの親の GUID（親を持たない・親が削除済みなら `null`）。同じ親を持つ項目が兄弟レーン。UI の一覧で入れ子に表示されている関係と同じもの |
| `name` | string | 表示名 |
...
>
> **親がアーカイブされている子セッションは、一覧に載せない**（UI に合わせた）。子自身はアーカイブされていなくても、UI の一覧では親行ごと姿を消す（親はアーカイブ除外で親行の候補から外れ、一方で孤児判定は「親が存在するか」しか見ないため子は孤児にもならない）。人間から見えないセッションは復元も起動もしてもらえないので、宛先として出しても無駄に終わる。
>
> 対して**親が削除された子は載せる**。UI が孤児として最上位に並べるのと同じ扱いで、`parentSessionId` は `null` で返す。
>
...
| 条件 | 付与される文字列 |
|---|---|
| 本人確定 + `contextId` | `[TerminalHub 自動メッセージ #<id12> — 依頼元: <名前> (<GUID>) / contextId: <id> — 回答・結果は update_context に status=completed で書く(working は依頼元に通知されない)。途中で相談したいときだけ send_to_session で依頼元へ返す]` |
