...
- **Codex → 起動オプション `-c mcp_servers.terminalhub.url=<URL>` で渡せる**ことを実測で確認済み（設定ファイルへの書き込み不要・既存 MCP とマージされる。
  値は TOML としてパースされ、失敗すればリテラル文字列として扱われるため URL は引用符なしでそのまま渡せる）。**この方式へ移行予定**。
  現状のコードは `<folder>/.codex/config.toml` の `[mcp_servers.terminalhub]`（`url`）へ追記する書き込み型で、既存設定はマージで温存し `terminalhub` エントリのみ所有する。
- ポートは実行中の値を反映。
- 実装: `TerminalHub/Services/McpConfigService.cs`（JSON 生成）、`TerminalHub/Constants/TerminalConstants.cs`（`BuildClaudeCodeArgs`）、
  `TerminalHub/Services/SessionManager.cs`（`ResolveClaudeMcpConfigPath`）。設定は `AppSettings.Experimental.AutoRegisterMcp`。
