...
>
> **親を辿れない子セッションは一覧に載せない**。宛先として出しても無駄に終わるためで、次の2つが該当する。
>
> - **親がアーカイブ済み**: 子自身はアーカイブされていなくても、UI の一覧では親行ごと姿を消す（親はアーカイブ除外で親行の候補から外れ、一方で孤児判定は「親が存在するか」しか見ないため子は孤児にもならない）。人間から見えないセッションは復元も起動もしてもらえない。
> - **親が削除済み（孤児）**: 削除は子へカスケードするので本来この状態は作られないが、それ以前に溜まった孤児が残るため念のため弾く。
>
> 結果として、`parentSessionId` に値が入っていれば**その親は必ず実在する**。ただし呼び出し側のフィルタ引数で親が落ちれば、その応答の中には現れない（フィルタは1件ずつ独立に効くため）。
...
> **`target` は `targetSessionId` に改名し、表示名での宛先指定を廃止した**（2026-08-08・非互換）。表示名は実運用で普通に重複する（worktree の子セッションはブランチ名が似る、`DisplayName` 未設定だとフォルダ名にフォールバックする）のに、旧実装は先頭一致で黙って1件を選んでいた＝**別セッションへ送っても成功として返る**穴だった。誤送信は送れてしまうぶん静かに失敗するため、互換は残さず弾いて GUID を引き直させる。引数名に `target` を残さず向きを明示したのは、エージェントが自分の GUID（`TERMINALHUB_SESSION_ID`）を渡す取り違えを防ぐため。GUID 以外を渡すと `list_sessions` へ誘導するメッセージ付きで `success=false` になる。
>
...
- **再送してよい**（未起動・待ち行列が上限・宛先が見つからない・`targetSessionId` が GUID でない）
- **同じ内容を再送してはいけない**（書き込みが途中で失敗した）。本文だけが相手の入力欄へ届いている可能性があり、再送すると二重に連結される

**`success=false` になるケース**（例外にせず結果で返す）

- `targetSessionId` が **GUID ではない**（表示名を渡した等）。`list_sessions` で GUID を引き直させる
- 宛先セッションが見つからない
...
| 条件 | 付与される文字列 |
|---|---|
| 本人確定 + `contextId` | `[TerminalHub 自動メッセージ #<id12> — 依頼元: <名前> (<GUID>) / contextId: <id> — 回答・結果は update_context に status=completed で書く(working は依頼元に通知されない)。この依頼を終えるまで人間への選択肢(AskUserQuestion等)は控え、確認・相談は send_to_session で依頼元へ返す]` |
...
**終端 → 同一終端（completed → completed 等）の再書き込みも通知する（2026-08-02 変更）。** 当初は「遷移を成立させたときだけ通知」だったが、完了後に人間が受け手セッションで追加調整をさせて「再完了」を書いたとき、その続報が依頼元に伝わらない実害があった。現在は `update_context` で**終端 status の書き込みが成功すれば毎回通知**する（要約と記名も更新される）。代償として同時に completed を書いた2者がいると依頼元が二度起きることがあるが、通知は status と contextId を運ぶだけで内容は `get_context` で読むため実害は薄い。ただし**依頼元自身の書き込みには通知しない**: 自己書き込みにも通知すると「自分へ再通知 → また書く」の自己励振ループが成立してしまう（判定は `ContextNotifyPolicy` に分離しテストで固定）。かつては通知文面自体が「必要なことは update_context に書いて終えよ」とループを誘発していた。文面は報告のみに改めた（2026-08-08。→「完了通知」節）が、依頼元も contextId を知るキー付きセッションである以上いつでも札へ書けるため、この防波堤は維持する。なお**システム経路（配送失敗の failed 化）は従来どおり遷移時のみ**（機械的な失敗記録の重複で依頼元を起こす価値がないため）。
...
- **`update_context(contextId, summary, status?)`**: 受け手が状況・結果を書く。summary は
  **要約1枚の全体上書き**（履歴は積まれない）。長い成果物はファイルに書いてパスを載せる。
  ...
  書き込みは**接続キーで本人確定できる接続のみ**。自動的に記名され、「どのセッションが書いたか」が
  検証済みで記録される（`updatedBy`）。本人確定の無い接続（外部クライアント等）は**書けない＝読み取り専用**
  （2026-08-08 変更。従来は「キーは ACL でなく署名」として無記名でも書けたが、contextId は受け手の
  ターミナルに平文で流れるため、ID を見ただけの第三者が偽の完了報告＝終端 status を書けて
  依頼元へ通知まで飛ぶ穴だった。無記名書き込みの実利用も無かった。理論上失うのは
  「外部クライアントによる自依頼のキャンセル」だけで、必要になれば発行時に資格を返す形で足せる）
- **`get_context(contextId)`**: 札を読む。依頼元が本人確定済みのセッションなら終端 status が書かれた時点で自動通知が届くのでポーリングは不要。無記名の外部クライアントはこれをポーリングする。
  `updatedBy` で「本当に依頼先セッションが書いた結果か」を確認できる（`TerminalHub (system)` は
  配送失敗等のシステム記録。null は旧仕様時代の無記名書き込み）
- **contextId は capability 兼用**（知っている＝読める。deliveryId と同じ哲学で認証を別途作らない。
  書き込みはさらに本人確定が必要＝上記）
...
```
[TerminalHub] 依頼(contextId: xxx) completed（担当: レビュー係）: 指摘8件すべて修正、ビルド通過 (応答不要)
```

**「誰が」と「結果」を本文に載せる**（2026-08-20 文面変更）。担当は終端 status を書いたセッション名
（`ContextRecord.UpdatedByName`。無記名の書き込みなら「外部クライアント」）、結果はその要約
（`Summary`）で、改行・制御文字は空白へ潰し 120 文字で切り詰める（エンベロープ付きの1行として送るため。
切り詰め位置がサロゲートペアの途中に当たる場合は1つ手前で切る）。**担当名も同じサニタイズを通す** —
表示名は人間の自由入力で、`\r` が混ざれば受け手の TUI がそこを送信確定と解釈し、エンベロープより手前だけが
「マーカー無し＝人間の指示」として実行される（PR #188 で本文について塞いだのと同じ迂回）。
ID だけの旧文面は、札を複数抱えた依頼元が照合するまで何の通知か分からず、結果を見るのに必ず
`get_context` が要った。`get_context` への誘導は**要約が無い/切り詰めたときだけ**足す（常に書くと、
本文で足りているケースでも律儀に叩かれてターンを1つ余計に消費する）。

「この通知はあなたへの報告」「結果を踏まえて元の作業を続けよ」の説明は本文から落とし、
`send_to_session` のツール説明へ移した（ツール定義はセッション中ずっとコンテキストに載るので、
通知ごとに書くより確実に効く）。本文に残すのは `(応答不要)` だけ。

**通知は報告に徹する**（2026-08-08 文面変更）。旧文面の「必要なことは update_context に書いて終えてください」は
依頼元に受け手の作法を指示する混入で、従うと受け手の結果要約を上書きしてしまう
（自己励振ループの誘発源でもあった＝ `ContextNotifyPolicy` の防波堤はその名残として維持）。
「選択肢を出すな」もやり取りが終わった時点の依頼元にはスコープを決められない指示だった。
選択肢を控える案内は、依頼開始時の受け手側エンベロープが「この依頼を終えるまで」と
スコープ付きで持つ（→「本文末尾に付く定型文」の表）。なお効くのはエージェント自身が出す選択肢
（`AskUserQuestion` 等）だけで、権限プロンプトは設定とツールポリシーが出すもので抑えられない。
塞がった場合の受け皿は配送キューのほう。
...
**受け手側の作法として「札は依頼単位」を明示する（2026-08-02 追記）。** 実運用で、受け手が
質問への回答を `working` のまま札に書き「自分の残作業が終わったら completed にする」運用にした結果、
**回答が書かれているのに依頼元は永遠に気づけない**事故が起きた。エンベロープ・instructions・
`update_context` の説明はいずれも「依頼（質問・作業）に応えられた時点で completed にする。
受け手の残作業は札の完了とは別（終わったら再度 completed ＝再完了の続報として再通知される）。
working は依頼元に通知されない」と明記する（文言のみの対処。通知挙動は変えない）。
