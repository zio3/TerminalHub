...
**終端 → 同一終端（completed → completed 等）の再書き込みも通知する（2026-08-02 変更）。** 当初は「遷移を成立させたときだけ通知」だったが、完了後に人間が受け手セッションで追加調整をさせて「再完了」を書いたとき、その続報が依頼元に伝わらない実害があった。現在は `update_context` で**終端 status の書き込みが成功すれば毎回通知**する（要約と記名も更新される）。代償として同時に completed を書いた2者がいると依頼元が二度起きることがあるが、通知は status と contextId を運ぶだけで内容は `get_context` で読むため実害は薄い。なお**システム経路（配送失敗の failed 化）は従来どおり遷移時のみ**（機械的な失敗記録の重複で依頼元を起こす価値がないため）。
