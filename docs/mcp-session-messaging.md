...
| `proof` | string? | なし | 本人証明（`TERMINALHUB_SESSION_PROOF`）。渡すと**検証済みの送信元が本文末尾に付き**、`contextId` と併用すれば**完了時に自分のセッションへ通知が届く** |
...
**入力待ち（`waiting_user_input`）は失敗にならない**。受理して配送キューへ積み、待ちが解消してから送る（→ 次節）。
