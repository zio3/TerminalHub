...
このファイルは、Codex CLI がこのリポジトリで作業する際のガイダンスを提供します。
...
2. **セッションタイプ**
   ...
   - Codex CLIセッション（出力解析付き）
   - Antigravity (agy) CLIセッション（出力解析なし、最小サポート）
   - Grok CLIセッション（出力解析なし、最小サポート）
   - DOSコマンドセッション
...
4. **主要サービス**
   - `OutputAnalyzerService`: Claude Code / Gemini / Codex CLI 用出力解析、処理状態追跡
...
5. **出力解析**: Claude Code / Gemini / Codex CLI 出力のトークン使用量と処理時間のリアルタイム解析
