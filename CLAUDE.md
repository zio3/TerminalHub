...
7. **MCP サーバー（セッション間メッセージング）**
   ...
   - ContextSummary（context）: 依頼単位の「状況札」。`send_to_session` の `contextId="new"` で発行（A2A の contextId/TaskState に対応）。依頼元が本人確定済みのセッションなら**終端 status の書き込み時に自動通知**（同一終端への再書き込み＝再完了の続報も再送）、外部クライアントは `get_context` のポーリングで受け取る。詳細は `docs/mcp-session-messaging.md`
