...
```json
{
  "eventType": "UserPromptSubmit | Stop | SubagentStart | SubagentStop | PreCompact | PostCompact",
  "tool": null | "ClaudeCode",
  "sessionId": "Session_{guid} | SubAgent_{agent_id}",
  "sessionName": "セッション名",
...
| フィールド | 型 | 説明 |
|-----------|------|------|
| `eventType` | string | 発火した hook イベント名そのまま（`UserPromptSubmit` / `Stop` / `SubagentStart` / `SubagentStop` / `PreCompact` / `PostCompact`） |
| `tool` | string? | 送信元 CLI 名（スペース無し）。Claude Code の hook 由来は `"ClaudeCode"`。指定が無い送信は `null` |
| `sessionId` | string | 種別プレフィックス付きキー。通常セッションは `Session_{GUID}`、サブエージェントは `SubAgent_{agent_id}`（受信側で両者を区別するため） |
...
- サブエージェントの開始/終了は `sessionId` が `SubAgent_{agent_id}` になるため、個別 LED として扱える。
