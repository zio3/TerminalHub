...
| フィールド | 型 | 説明 |
|-----------|------|------|
| `eventType` | string | 発火した hook イベント名そのまま（`UserPromptSubmit` / `Stop` / `SubagentStart` / `SubagentStop` / `PreCompact` / `PostCompact` / `Notification` / `PreToolUse`） |
...
| `elapsedSeconds` | int? | 処理時間（秒）。`Stop` のみ値が入り、それ以外は `null` |
| `elapsedMinutes` | float? | 処理時間（分）。`Stop` のみ値が入り、それ以外は `null` |
