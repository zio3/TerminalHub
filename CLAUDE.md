...
> `TerminalHub.exe --notify --source codex` をブリッジ起動し、stdin の JSON を `/api/hook/codex/{guid}` へ
> 転送する（`CodexHookService` が `.codex/hooks.json` に登録）。受信後は Claude と同じ `HookNotificationService`
> に流れ、`eventType` は本来の hook 名そのまま・`tool="CodexCLI"` で送る。登録イベントは Stop / UserPromptSubmit /
> SubagentStart / SubagentStop / PreCompact / PostCompact / PermissionRequest（許可待ち。`toolName` 付き）。
> （PreToolUse / PostToolUse / SessionStart は当面未登録）
>
> Hook を持たない CLI（Gemini / Terminal 等）は `OutputAnalyzerService` の出力解析ベースで
> `eventType` は従来どおり **`"start"` / `"complete"`**（このとき `tool` は `null`）を送る。
