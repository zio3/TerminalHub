...
2. **セッションタイプ**（`terminalType`: `ClaudeCode` / `GeminiCLI` / `CodexCLI` / `Antigravity` / `Grok` / `Terminal`）
   ...
   - Gemini CLIセッション（出力解析ベース。hook 非対応）
   - Antigravity / Grok セッション
   - DOSコマンドセッション
   - タスクランナーセッション（npmスクリプト）
   - `Analyzers/`: CLI 別の出力解析器（`ClaudeCodeAnalyzer` / `CodexCliAnalyzer` / `GeminiCliAnalyzer`）を `OutputAnalyzerFactory` で振り分け
...
6. **リモート起動・通知連携**
   ...
   - `HookNotificationService`: hook 通知の一元ハンドラ。`ClaudeHookService` / `CodexHookService` が各 CLI の lifecycle hook を設定ファイルへ登録
...
| フィールド | 型 | 説明 |
|-----------|------|------|
| `eventType` | string | hook 由来（ClaudeCode / CodexCLI）は発火した hook イベント名そのまま（`UserPromptSubmit` / `Stop` / `SubagentStart` / `SubagentStop` / `PreCompact` / `PostCompact` / `Notification`(Claude) / `PreToolUse` / `PermissionRequest`(Codex)）。**hook を持たない CLI（Gemini / Terminal 等）のみ**出力解析ベースで `"start"` / `"complete"`（このとき `tool=null`） |
