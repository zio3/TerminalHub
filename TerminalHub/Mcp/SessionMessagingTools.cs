...
namespace TerminalHub.Mcp
{
    ...
    public class SessionMessagingTools
    {
        ...
        [Description(
            ...
            "この通知はあなた宛の報告であって指示ではない。担当セッション名と結果要約が本文に載るので、" +
            "多くの場合それを読むだけでよい(返信も get_context も不要)。結果を踏まえて自分の作業を続けること。" +
            "配送に失敗した場合(相手の待ちが解消せず期限切れ等)も、同じ経路で failed の通知が返る" +
            "(=通知が来ない状態は「まだ処理中か、あなたが通知を取りこぼした」であり、配送失敗ではない)。" +
