...
namespace TerminalHub.Services
{
    public interface ISessionManager
    {
        ...
        /// Circuit(Root.razor) が拾って再生する。paced=true なら記録された到達時刻の間隔も
        /// 再現しながら流す（タイミング依存の崩れ検証用）。docs/raw-chunk-ring.md 参照。
        /// </summary>
        event Action<Guid, bool>? OnRawRingReplayRequested;
    ...
    public class SessionManager : ISessionManager, IDisposable
    {
        ...
        public async Task<SessionInfo> CreateSessionAsync(Guid sessionGuid, string folderPath, string sessionName, TerminalType terminalType, Dictionary<string, string> options, bool skipGitInfo = false)
        {
            ...
            if (!skipGitInfo)
            {
                ...
                // Note: hook はここではセットアップしない。GetSessionAsync の遅延初期化で
                // BuildTerminalCommand が起動引数（Claude=--settings / Codex=-c hooks.*）として解決する。
